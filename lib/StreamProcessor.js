...
//cat
createProcessor('cat',
function(callback, context, stream1, stream2) {
...
createProcessor('highlight', function(callback, context, stream){
    ...
    function() {
		...
		var html = '<html><head>';
			html+= '<link rel="stylesheet" href="highlightstyle.css"/>';
			html+= '</head>';
			html+= '<body><code>'+highlight(source)+'</code></body></html>';
        out.write(html);
        out.end();
    });
    callback(out);
